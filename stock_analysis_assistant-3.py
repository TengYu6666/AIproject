    ...
    def get_historical_data(self, ts_code):
        ...
        try:
            engine = create_engine(
                f'mysql+pymysql://{mysql_user}:{mysql_password}@{mysql_host}:{mysql_port}/{database}?charset=utf8mb4',
...
class ExcSQLTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            error_msg += "\n\n请检查："
            error_msg += "\n1. MySQL 服务是否已启动"
            error_msg += f"\n2. 当前使用的连接参数: 用户名={mysql_user}, 主机={mysql_host}, 端口={mysql_port}, 数据库={database}"
